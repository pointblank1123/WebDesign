...
<body>
    ...
    <?php
        ...
        $res = $conn->query($sql);
        if($res->num_rows > 0){
            echo "<table style='color:white'><tr><th>ID</th><th>Vehicle Presence</th><th>Vehicle Type</th><th>Human Harm</th><th></th>Environment Harm</th><th>Physical Trace</th><th>Trace Description</th><th>Additional Details</th></tr>";
            while($row = $res->fetch_assoc()){
                ...
                '<td>'.$row['descriptionOfTrace'].'<td>'.$row['additionalDetails'].'</td></tr>';  
            }
            echo '</table>';
        }
        else{
